...
**重要:** このワークフローは、可能な限り自動で実行されるように設計されています。各ステップは完了後、ただちに次のステップへ移行してください。ただし、以下の場合はユーザーに確認を求めてください：
- テスト・lint・typecheckの失敗修正が3回連続で解決しない場合
- アーキテクチャや設計方針の根本的な変更が必要になった場合
- 外部サービスやAPIの障害など、コードでは解決できない問題が発生した場合
...
1. Grepツールを使用し、機能名に関連するキーワードでソースコードを検索する。
  ```bash
  Grep('[機能に関連するキーワード]', 'app/')
  Grep('[機能に関連するキーワード]', 'lib/')
  Grep('[機能に関連するキーワード]', 'components/')
  ```
...
- **ルールA: タスクが大きすぎる場合**
  - 現在のタスクをより小さな複数のサブタスクに分割する。`Edit`ツールを使い、元のタスクを削除し、その場所に新しいサブタスク（`[ ]`付き）を挿入する。その後、ループを継続する。

- **ルールB: 技術的理由でタスクが不要になった場合**
  - 条件: 実装方針の変更、アーキテクチャの変更、依存関係の変更など、明確な技術的理由がある場合にのみ適用。
  - `Edit`ツールを使い、該当タスクを `[x] ~~タスク名~~ (理由: [具体的な技術的理由を簡潔に記述])` の形式で更新する。その後、ループを継続する。
...
3. **3回修正しても解決しない場合は、ユーザーに状況を報告し判断を求める。**
